...
export class DeepSeekService extends AIService {
  ...
  constructor(apiKey: string) {
    super(apiKey, 'deepseek-v3');
  }

  /**
   * 调用 DeepSeek API
   */
  private async callDeepSeek(messages: any[], temperature = 0.7): Promise<string> {
    try {
...
确保返回的是纯 JSON 格式，不要包含 markdown 标记。`;
    ...
    const result = await this.callDeepSeek([{ role: 'user', content: prompt }]);
...
以 JSON 格式返回完整脚本（与原脚本相同的结构）。`;
  ...
  async testConnection(): Promise<boolean> {
    try {
      await this.callDeepSeek([{ role: 'user', content: '测试连接' }], 0.1);
      return true;
    } catch (error) {
      return false;
    }
